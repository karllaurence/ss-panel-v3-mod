						...
						<div class="card margin-bottom-no">
							<div class="card-main">
								<div class="card-inner">
									<h4>注意：</h4>
									<p>请勿在任何地方公开节点地址。</p>
									<p>流量比例为 0.5 即使用 1000MB 按照 500MB 流量记录记录结算。</p>
																		...
																		{foreach $node_muport as $single_muport}
																			...
																			<div class="card">
																				<div class="card-main">
																					<div class="card-inner">
																					<p class="card-heading" >
																						<a href="javascript:void(0);" onClick="urlChange('{$node->id}',{$single_muport['server']->server},{if $relay_rule != null}{$relay_rule->id}{else}0{/if})">{$node_name_split[0]} {if $relay_rule != null} - {$relay_rule->dist_node()->name}{/if} - {$single_muport['server']->server} 端口</a>
																						<span class="label label-brand-accent">{$node->status}</span>
																					</p>
...
function urlChange(id,is_mu,rule_id) {
	...
	if(id == 'guide')
	{
		...
		doc.open();
		doc.write('<img src="/downloads/1469595156fca44223cf8da9719e1d084439782b27.gif" style="width: 100%;height: 100%; border: none;"/>');
		doc.close();
	}
	else
	{
